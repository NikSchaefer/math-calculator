        ...
        case TokenType.Caret:
            if (children.length < 2) {
                throw new ParseError(
                    "Expected two children for ^ operator",
                    token
                );
            }
            ...
            if (children[1].isSymbolNode && children[1].name === "T") {
                return new (math as any).FunctionNode("transpose", [
                    children[0],
                ]);
            }
            return new (math as any).OperatorNode(token.lexeme, fn, children);
...
class Parser {
    ...
    nextTerm(): math.MathNode {
        ...
        for (;;) {
            ...
            if (
                isNumberNode(leftFactor) &&
                lookaheadType === TokenType.Number
            ) {
                throw new ParseError(
                ...
                this.match(TokenType.Star, TokenType.Times, TokenType.Slash)
            ) {
        ...
        switch (lookaheadType) {
            ...
            case TokenType.T:
                ...
                if (this.match(TokenType.Underscore)) {
                    ...
                    // Get the subscript (either a single variable/number or a group in braces)
                    let subscript;
                    if (this.match(TokenType.Lbrace)) {
                        this.nextToken(); // consume {
                        subscript = this.nextExpression();
                        this.tryConsume(
                    ...
                    // Combine the variable and subscript into a single symbol name
                    const baseNode = primary as any;
                    const subscriptNode = subscript as any;
                    const combinedName =
                        baseNode.name +
                        "_" +
                        (subscriptNode.name || subscriptNode.value);
