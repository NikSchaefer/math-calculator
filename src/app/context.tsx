...
export function CalculatorProvider({ children }: { children: ReactNode }) {
  ...
  const { computedCalculators, combinedContext } = useMemo(() => {
    ...
    // Second pass: Recalculate with known variables, maintaining ans between calculations
    const computedCalculators: ComputedCalculator[] = [];
    for (const calculator of calculators) {
      const computed = computeCalculator(calculator, combinedContext);
      computedCalculators.push(computed);

      // Update ans with the last result if it exists and is a number
      if (computed.results.length > 0 && computed.results[0].result !== null) {
        const result = computed.results[0].result;
        if (typeof result === "number") {
          combinedContext.ans = result;
        }
      }
    }
  ...
  const onUseItem = useCallback(
    (item: Preset | PresetVariable | null) => {
      ...
      if ("calculators" in item) {
        ...
        const isPreviousCalculatorEmpty = calculators[length - 1].latex === "";
